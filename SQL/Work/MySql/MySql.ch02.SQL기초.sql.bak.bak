...
-- AS로 컬럼에 별칭 부여하기. 
-- emp 테이블에서 job 컬럼에는 직책을 , enname 컬럼에 이름이라는 별칭을 붙여 출력하시오.
...
select ename, ifnull(mgr,0) from emp;-- null값을 0으로 바꿈
...
-- emp테이블에서 enmae만 출력하되 중복되는 ename을 제거하시오. 18개
...
-- 와일드 카드 "_"를 가운데 글자에 "성"자가 들어 있는 사람을 출력하시오.
select * from emp where ename like '_성_';

-- 와일드 카드 "*"를 사용하여 "성"자로 끝나는 사람을  출력하시오.
select * from emp where ename like '%성';
...
-- LIKE '%a'	:  a로 끝나는
-- LIKE '%or%'	:  or무조건 찾아라?
-- LIKE '_r%'	: 1r, 1ra, 2rr  <--첫번째는 상관 없지만 두 번째는 반드시 r
-- LIKE 'a_%_%' : 
-- LIKE 'a%o'	: ao, a1a, a.......~
...
select * from emp where comm is null; -- is null는 자바에서 ==과 같다.
...
insert into emp(depto, ename, job)
				values(30, '송혜교, '과장');
