...
-- 오늘은 이번달의 몇 주차인가? 
-- MYSQL  : dayofweek()
-- Oracle : to_char(sysdate, 'IW')--올해의 주차 'w'는 이번달 주차
 
select sysdate, to_char(sysdate, 'W') from emp; --이번달 주차
select sysdate, to_char(sysdate, 'IW') from emp; --올해 주차
...
-- 문제. 문자 20,000,000.73797874857848 을 실수로 바꾸시오. 변환전에 ','를 반드시 삭제 해야만 한다.
...
select nvl ('0', 1) from dual; --문자열 0 출력, 문자열은 좌측 정렬로 표시
